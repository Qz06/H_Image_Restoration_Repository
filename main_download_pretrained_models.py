...
def download_pretrained_model(model_dir='model_zoo', model_name='dncnn3.pth'):
    ...
    if os.path.exists(target_path) and os.path.getsize(target_path) > 0:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                with requests.get(url, allow_redirects=True, stream=True, timeout=timeout) as response:
                ...
                if attempt == max_retries:
                    print(f'failed to download [{model_name}] after {max_retries} attempts: {exc}')
                    return False
                wait_seconds = 2 ** (attempt - 1)
                print(f'download failed (attempt {attempt}/{max_retries}): {exc}; retrying in {wait_seconds}s ...')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--models',
                        type=lambda s: [x for x in re.split(r'[\s,]+', s.strip()) if x],
                        default=['dncnn3.pth'],
