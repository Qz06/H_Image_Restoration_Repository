...
class ModelPlain(ModelBase):
    ...
    def test(self, use_ema=False):
        net = self.netE if (use_ema and hasattr(self, 'netE')) else self.netG
    ...
    # test / inference x8
    # ----------------------------------------
    def testx8(self, use_ema=False):
        net = self.netE if (use_ema and hasattr(self, 'netE')) else self.netG
        was_training = net.training
        net.eval()
        with torch.no_grad():
            self.E = test_mode(net, self.L, mode=3, sf=self.opt['scale'], modulo=1)
        if was_training:
            net.train()
