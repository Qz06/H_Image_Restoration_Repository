...
def _load_json_with_comments(opt_path):
    json_lines = []
    with open(opt_path, 'r') as f:
        for line in f:
            json_lines.append(line.split('//')[0] + '\n')
    return json.loads(''.join(json_lines), object_pairs_hook=OrderedDict)


def _expand_dataset_paths(dataset):
    for key in ['dataroot_H', 'dataroot_L', 'dataroot_gt', 'dataroot_lq', 'meta_info_file']:
...
def parse(opt_path, is_train=True):
    ...
    # ----------------------------------------
    opt = _load_json_with_comments(opt_path)
    ...
    # network
    # ----------------------------------------
    opt['netG'].setdefault('scale', opt['scale'] if 'scale' in opt else 1)
    if 'scale' in opt:
        opt['netG'].setdefault('upscale', opt['scale'])
    ...
    if 'netD' in opt and 'D_optimizer_wd' not in opt['train']:
        opt['train']['D_optimizer_wd'] = 0
    ...
    if 'E_decay' not in opt['train']:
        opt['train']['E_decay'] = 0
    if 'val_which_model' not in opt['train']:
        opt['train']['val_which_model'] = 'G'
